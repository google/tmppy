...
def remove_unused_toplevel_elems(header: ir0.Header, linking_final_header: bool):
    toplevel_elem_names = {elem.name
                           for elem in itertools.chain(header.toplevel_content, header.template_defns)
                           if not isinstance(elem, ir0.StaticAssert)}
    ...
    for elem in itertools.chain(header.template_defns, header.toplevel_content):
        if isinstance(elem, (ir0.TemplateDefn, ir0.ConstantDef, ir0.Typedef)):
            elem_name = elem.name
        else:
        ...
        if elem_name in public_names or (isinstance(elem, (ir0.ConstantDef, ir0.Typedef)) and any(isinstance(expr, ir0.TemplateInstantiation) and expr.instantiation_might_trigger_static_asserts
    ...
    new_header = ir0.Header(template_defns=[template_defn for template_defn in header.template_defns if
                                            template_defn.name in used_elem_names],
                            toplevel_content=[elem for elem in header.toplevel_content if
                                              isinstance(elem, ir0.StaticAssert) or elem.name in used_elem_names],
                            public_names=header.public_names,
