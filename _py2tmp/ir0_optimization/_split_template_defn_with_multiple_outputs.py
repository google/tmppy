...
def split_template_defn_with_multiple_outputs(template_defn: ir.TemplateDefn,
                                              ...
                                              identifier_generator: Iterator[str]) -> Tuple[List[
                                                                                                ir.TemplateDefn], bool]:
    ...
    for result_elem_name in actual_result_elem_names:
        ...
        if expr_type.kind in (ir.ExprKind.TYPE, ir.ExprKind.TEMPLATE):
            stmt_type = ir.Typedef
        else:
            stmt_type = ir.ConstantDef
        ...
        dispatcher_body.append(stmt_type(name=result_elem_name,
                                         expr=ir.ClassMemberAccess(class_type_expr=ir.TemplateInstantiation(template_expr=split_template_literal,
                                                                                                            args=[ir.AtomicTypeLiteral.for_local(cpp_type=arg.name,
...
class ReplaceMetafunctionCallWithSplitTemplateCallTransformation(Transformation):
    ...
    def transform_class_member_access(self, class_member_access: ir.ClassMemberAccess):
        if (isinstance(class_member_access.expr, ir.TemplateInstantiation)
            ...
            class_member_access = ir.ClassMemberAccess(class_type_expr=ir.TemplateInstantiation(template_expr=ir.AtomicTypeLiteral.for_nonlocal_template(cpp_type=split_template_name,
