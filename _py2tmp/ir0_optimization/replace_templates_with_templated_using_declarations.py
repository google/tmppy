...
def _is_trivial_pattern(arg_decl: ir0.TemplateArgDecl, pattern: ir0.Expr):
    if isinstance(pattern, ir0.AtomicTypeLiteral):
        ...
        return arg_decl.is_variadic and arg_decl.name == pattern.expr.cpp_type
    else:
        return False
...
class _DetermineTemplatesThatCanBeReplaced(Visitor):
    ...
    def visit_class_member_access(self, class_member_access: ir0.ClassMemberAccess):
        if (isinstance(class_member_access.expr, ir0.TemplateInstantiation)
            ...
            # AtomicTypeLiteral.
            super().visit_exprs(class_member_access.expr.args)
        else:
...
class _ApplyReplacement(Transformation):
    def __init__(self,
        ...
        self.additional_typedef_args_in_current_template: List[ir0.TemplateArgDecl] = []
    ...
    def transform_template_defn(self, template_defn: ir0.TemplateDefn):
        ...
        self.additional_typedef_args_in_current_template = [arg_decl
    ...
    def transform_template_specialization(self, specialization: ir.TemplateSpecialization):
        ...
        args = [self.transform_template_arg_decl(arg_decl)
    ...
    def transform_type_literal(self, type_literal: ir.AtomicTypeLiteral):
        if self.additional_typedef_args_in_current_template and type_literal.cpp_type in self.locals_to_instantiate:
            ...
            return ir0.TemplateInstantiation(template_expr=ir.AtomicTypeLiteral.for_local(cpp_type=type_literal.cpp_type,
                                                                                          expr_type=ir0.TemplateType([ir0.TemplateArgType(expr_type=arg.expr_type,
    ...
    def transform_class_member_access(self, class_member_access: ir0.ClassMemberAccess):
        if (isinstance(class_member_access.expr, ir0.TemplateInstantiation)
                and isinstance(class_member_access.expr.template_expr, ir0.AtomicTypeLiteral)
                and class_member_access.expr.template_expr.cpp_type in self.movable_arg_indexes_by_template_name):
            assert isinstance(class_member_access.expr.template_expr.expr_type, ir0.TemplateType)
            # F<X, Y>::type -> F<X>::type<Y>  (if X is used in non-trivial patterns and Y isn't)

            args = self.transform_exprs(class_member_access.expr.args, class_member_access.expr)

            movable_arg_indexes = self.movable_arg_indexes_by_template_name[class_member_access.expr.template_expr.cpp_type]
            template_instantiation_arg_exprs = [arg
                                                for index, arg in enumerate(args)
                                                if index not in movable_arg_indexes]
            typedef_instantiation_arg_exprs = [arg
                                               for index, arg in enumerate(args)
                                               if index in movable_arg_indexes]
            typedef_instantiation_arg_types = [arg_type
                                               for index, arg_type in enumerate(class_member_access.expr.template_expr.expr_type.args)
                                               if index in movable_arg_indexes]

            template_instantiation = ir0.TemplateInstantiation(template_expr=ir0.AtomicTypeLiteral.for_nonlocal_template(cpp_type=class_member_access.expr.template_expr.cpp_type,
                                                                                                                         args=[arg
                                                                                                                               for index, arg in enumerate(class_member_access.expr.template_expr.expr_type.args)
                                                                                                                               if index not in movable_arg_indexes],
            ...
            new_class_member_access = ir0.ClassMemberAccess(class_type_expr=template_instantiation,
                                                            member_name=class_member_access.member_name,
                                                            member_type=ir0.TemplateType(typedef_instantiation_arg_types))
